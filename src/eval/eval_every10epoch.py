...
def eval_epoch(real_paths,fake_paths,epoch, path):
    ...
    for i in range(len(real_paths)):
        ...
        results_kid = calculate_kid_given_paths(paths_kid, batch_size=40, cuda=True, dims=2048, model_type='inception')
        rets[i].kid_mean=results_kid[0][1]
        rets[i]._kid_std = results_kid[0][2]
        results_fid_inkid = calculate_fid_given_paths(paths_kid, batch_size=40, cuda=True, dims=2048, model_type='inception')
